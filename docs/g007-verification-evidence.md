...
- The baseline had no browser harness proving responsive screenshots, automated accessibility, horizontal overflow, focus, forced colors, reduced motion, or Lighthouse scores.
...
- `npm test -- tests/unit/guides.test.tsx tests/unit/site-config.test.ts tests/unit/privacy-seo-operations.test.tsx`: covered by the final full-suite PASS, 24/24 across those files. This covers 30/90/180-day content classes, inclusive due-date behavior, the 14-day warning window, required published/draft state, draft exclusion from public collections, route-specific canonical metadata, verified-fact shape/provenance/date validation, and preview SEO fail-closed behavior.
...
- PASS — the intentional 404 returned 404 with one `main`, one `h1`, and zero serious or critical Axe findings.
- PASS — keyboard activation opened the mobile menu, moved focus into its first visible link, and retained a visible focus indicator.
- PASS — keyboard focus was visibly rendered and the 360px contact input was not obscured by the mobile sticky CTA.
- PASS — 200% root text reflow retained visible main content with no horizontal overflow at 360px.
- PASS — an empty native form submission focused the first invalid field with a browser validation message; the actual endpoint returned private/noindex 400 HTML without false-success copy.
...
The Playwright interception is explicitly a UI-navigation and recovery-copy simulation. It does not claim to generate the public outcome, validate request security, contact Discord, or prove delivery; the integration suite remains authoritative for those boundaries.
